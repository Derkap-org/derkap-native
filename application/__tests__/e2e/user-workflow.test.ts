...
describe("User Workflow E2E Tests", () => {
  ...
  describe("Complete User Journey", () => {
    test("should complete full user workflow: auth → friends → challenge → response", async () => {
      ...
      const authResult = await mockFunctions.signIn(
        "test@example.com",
        "password123",
      );
      ...
      await AsyncStorage.setItem(
        "user_session",
        JSON.stringify(authResult.data.session),
      );
      ...
      const searchResult =
        await mockFunctions.getUserAndCheckFriendship("newpotentialfriend");
      ...
      const friendRequestResult =
        await mockFunctions.insertFriendRequest("new-user-789");
      ...
      const keysResult = await mockFunctions.generateKeys({
      ...
      mockFunctions.getEncryptionKey.mockResolvedValue(
      ...
      expect(mockFunctions.signIn).toHaveBeenCalledWith(
        "test@example.com",
        "password123",
      );
      expect(mockFunctions.getFriends).toHaveBeenCalled();
      expect(mockFunctions.getUserAndCheckFriendship).toHaveBeenCalledWith(
        "newpotentialfriend",
      );
      expect(mockFunctions.insertFriendRequest).toHaveBeenCalledWith(
        "new-user-789",
      );
      expect(mockFunctions.generateKeys).toHaveBeenCalledWith({
        challenge: challengeText,
      });
  ...
  describe("Error Handling in Workflows", () => {
    ...
    test("should handle friend request already exists", async () => {
      ...
      await expect(
        mockFunctions.insertFriendRequest("existing-friend-id"),
    ...
    test("should handle encryption key generation failure", async () => {
      ...
      await expect(
        mockFunctions.generateKeys({ challenge: "Test challenge" }),
      ).rejects.toThrow("API Error: Unable to generate keys");
    });
  });
